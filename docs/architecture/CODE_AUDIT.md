...
(both destined for `docs/report/REPORT.md`). No source was modified. Method: AST walk (docstring/typing
...
| 4a | `from __future__ import annotations` | ✅ **~90 %** — 28/31 real `src/` files + all `tools/` + both apps | 3 one-line adds: `analyze_benchmark.py`, `generate_synthetic_documents.py`, `model_config.py` |
...
**Not done (deferred):** the root `app.py`/`lab.py` and top-level `tools/` were left in place (a `cli/`
consolidation of the loose scripts is optional future work). **Coherence smell still open:** overlap
between `evaluation/` modules and `tools/` eval scripts (`tools/eval_document.py` etc.).
...
*(`tools/` — all 10 have `future`, 0 docstrings on their single public `main`; `tests/` — all 21
