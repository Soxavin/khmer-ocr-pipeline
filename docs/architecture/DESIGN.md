...
- **Confidence Trio** — **High** (`#16a34a`), **Mid** (`#f59e0b`), **Low** (`#dc2626`): The recognizer's own belief about a cell, mirrored 1:1 from the Python pipeline (`apps/api/components.py`). This is the trust palette; it is never used for anything but confidence.
...
Every control composes from one shared vocabulary (`apps/web/src/ui.ts`); per-instance styling drift is treated as the enemy. All interactive elements carry default / hover / focus / active / disabled, and press-feedback is a uniform `active:scale-[0.98]`.
...
- **Do** compose every control from `apps/web/src/ui.ts`; give each one default / hover / focus / active / disabled.
