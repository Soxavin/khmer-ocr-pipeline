...
(e.g. ARDB forms). Entry points: a **React review workspace** (`apps/web/`,
primary — served at `http://localhost:8600/app` by `uv run python -m
apps.api.main`, which also serves the **NiceGUI fallback UI** at `/`), an older
...
> evaluation framework, benchmark results), see `docs/decisions/PROJECT_LOG.md`.
...
- React 19 + Vite + TypeScript + Tailwind 4 + AG Grid community + TanStack Query (`apps/web/`) — primary review workspace, served at `/app` from the built `apps/web/dist` (build: `cd frontend && npm run build`, or `./dev.sh build`; dev: `./dev.sh` starts backend + Vite HMR at :5173/app/, proxying `/api` to :8600)
- NiceGUI (pinned `>=2.0,<3.0`) — fallback review UI (`apps/api/main.py`) AND the FastAPI host for the REST layer (`apps/api/api.py` registers routes on `nicegui.app`, a FastAPI subclass — one process, models load once); Streamlit >=1.35 — legacy UI (`app.py`)
- pytest — tests
...
`docs/decisions/PROJECT_LOG.md` §2.23–2.24.
...
worse on data grids — comparison only. See `docs/decisions/PROJECT_LOG.md` §2.40.
...
`apps/api/tables.py::stitch_grids` on the *edited* grids, chosen with `?combine=`
...
Three-zone analyst workspace (`apps/web/src/`): queue rail (`components/queue/`),
...
`docs/operations/GLOSSARY.md` §5 "Confidence ≠ accuracy" before treating one as evidence. Server state lives in
`apps/api/registry.py` (process-global `Document` dict + a global `run_lock` —
...
granular via `Progress.cancel_requested`). REST layer: `apps/api/api.py` — thin
...
Khmer rendering: bundled Noto Sans Khmer (`apps/web/src/assets/fonts/`, OFL),
`.khmer-content` line-height 1.9, adjustable size (A−/A+, localStorage).

## UI (`apps/api/main.py`, NiceGUI — fallback)
...
`uv run python -m apps.api.main` (port 8600). Modules: `settings.py` (`Settings`
...
- **UI changes** -> `apps/web/` (React, primary; iterate with `./dev.sh` for
  hot reload, `./dev.sh build` when :8600/app must serve the new bundle)
  + `apps/api/api.py` if the server must expose new data (TDD in
  `tests/test_webapp_api.py`); `apps/api/main.py` (NiceGUI fallback) or `app.py`
...
`docs/decisions/superpowers/plans/` and `docs/decisions/superpowers/specs/` contain the
