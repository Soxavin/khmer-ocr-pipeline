...
`research/layout_yolo/README.md` (the layout track). For a short, non-technical
...
uv run python tools/verify_corrections.py --inspect corrections
...
uv run python research/kiri_finetune/build_trainset.py \
    --out research/kiri_finetune/trainset_$(date +%Y%m) \
...
PYTORCH_ENABLE_MPS_FALLBACK=1 uv run python research/kiri_finetune/train_kiri.py \
    --trainset research/kiri_finetune/trainset_$(date +%Y%m)

# 6. REGRESSION GATE — the deploy decision
KHMER_KIRI_WEIGHTS=stock        uv run python research/kiri_finetune/gate_ab.py --tag baseline
KHMER_KIRI_WEIGHTS=<candidate>  uv run python research/kiri_finetune/gate_ab.py --tag candidate
```
...
- **Recognition only.** Corrections teach Kiri to *read* better. They do not fix
  ...
  (`research/layout_yolo/README.md`).
...
| loop demo + visual gate | **built** (`tools/verify_corrections.py`, both layout paths) |
