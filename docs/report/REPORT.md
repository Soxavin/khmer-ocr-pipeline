...
benchmark manifests, and the `eval/` harness. Figures in `docs/report/figures/`.*
...
an expected typeface limitation, not a pipeline defect. See `docs/report/figures/accuracy_by_font.png`.
...
order, not superiority. See `docs/report/figures/engine_comparison.png`. (Surya `run` `20260622_154407`;
...
fragmentation is the bottleneck.** See `docs/report/figures/table_fragmentation.png`.
...
`eval/loss_history.csv`; figures in `docs/report/figures/finetune_eval/`.)*
...
`docs/report/figures/finetune_eval/finetune_story_overview.png` — with the per-run detail behind them in
...
(7 of 9 rows hit the 4096-token generation cap). See `docs/report/figures/finetune_eval/parse_failure_rate_by_run.png`
and `docs/report/figures/finetune_eval/cer_by_run.png`.
...
`docs/report/figures/finetune_eval/real_doc_comparison.png`.
...
- Engines: `OCR_ENGINE=surya|tesseract|hybrid`. Figures: `docs/report/figures/`.
- **Fine-tuning (§4.10)**: training notebooks `tools/colab_gemma4_e2b_finetune.ipynb` /
  `tools/colab_qwen35_finetune.ipynb`; real-doc scoring `tools/eval_finetune_real.py` (run
  against predictions from `tools/colab_eval_real.ipynb`); run logs
  ...
  each one: `docs/report/figures/finetune_eval/README.md`. Regenerate all five figures via
  `tools/plot_run_metrics.py`, `tools/plot_real_doc_comparison.py` and
  `tools/plot_finetune_story.py` (exact commands in that README); all share
  `tools/_report_style.py` for palette, type scale and figure conventions.
  ...
  `docs/report/figures/finetune_eval/finetune_dashboard.png` (built by
  `tools/plot_finetune_dashboard.py` from the same drawing code, so it can't disagree with
