"""NiceGUI review UI — entrypoint.  Run: `uv run python -m apps.api.main`.

Presentation layer over the Khmer OCR pipeline. Imports and calls the same functions as
the Streamlit `app.py`; all heavy work runs off the event loop via `apps.api.runner`.
...
api.mount_frontend()  # serve the built React app at /app when apps/web/dist exists
