...
apps.api modules (`runner`, `state`, `settings`, `tables`, `downloads`, `edits`).

Import this module BEFORE `ui.run` (done in `apps.api.main`). The built React
bundle, when present at `apps/web/dist`, is mounted at `/app`.
"""
...
# existing patch("apps.api.api._probe_pages") calls across tests/test_webapp_api.py
# keep working.
...
def mount_frontend() -> None:
    """Serve the built React bundle at /app when apps/web/dist exists."""
    # __file__ is apps/api/api.py, so parent.parent is apps/ -> apps/web/dist.
