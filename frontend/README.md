...
page image, export JSON/CSV/Excel. Served at `/app` by the FastAPI backend in `../webapp/`.

See the repo root's `CONTEXT.md` for how this fits into the wider pipeline, and this
...
```bash
./dev.sh build     # rebuild frontend/dist so :8600/app serves the real (non-HMR) bundle
...
(`webapp/registry.py`) — the app is refresh-safe; reloading the tab keeps the queue, results,
