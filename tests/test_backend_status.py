...
import khmer_pipeline.backend_status as bs


def test_running_true_when_pgrep_finds_pids():
    # pgrep exits 0 and prints PIDs when matches exist
    with patch("khmer_pipeline.backend_status.subprocess.run") as run:
...
def test_running_false_when_pgrep_missing():
    ...
    with patch("khmer_pipeline.backend_status.subprocess.run", side_effect=FileNotFoundError):
        assert bs.llama_server_running() is False


def test_running_false_on_unexpected_error():
    with patch("khmer_pipeline.backend_status.subprocess.run", side_effect=OSError("boom")):
        assert bs.llama_server_running() is False
