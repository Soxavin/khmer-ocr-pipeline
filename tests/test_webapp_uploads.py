...
`upload_id` was `md5[:12]` in apps/api/api.py and `md5[:8]` in apps/api/main.py, so the
...
from apps.api import api, uploads
...
def test_api_probe_pages_is_the_shared_implementation():
    ...
    attribute of apps.api.api, because ~20 tests patch "apps.api.api._probe_pages"."""
    assert api._probe_pages is uploads.probe_pages


def test_both_uis_derive_the_same_id_for_the_same_bytes():
    data = _pdf_bytes()
    # The NiceGUI page body cannot be imported (apps.api.main calls ui.run() at
