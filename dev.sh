...
#   ./dev.sh build    rebuild apps/web/dist so :8600/app serves the real bundle
...
# apps/web/dist, so every edit would need a rebuild + hard refresh. Vite proxies
# /api to :8600 (see apps/web/vite.config.ts), so the dev server talks to the same
...
if lsof -ti:8600 >/dev/null 2>&1; then
  ...
  echo "  NOTE: edited apps/api/ or src/khmer_pipeline/? run ./dev.sh restart"
else
