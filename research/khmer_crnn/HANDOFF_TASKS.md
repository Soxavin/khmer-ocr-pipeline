...
`research/khmer_crnn/FINETUNING_PLAN.md` (§5), `research/khmer_crnn/train.py`,
...
**Deliverable B — local-corpus loading in `research/khmer_crnn/train.py`:**
...
2. `uv run python research/khmer_crnn/train.py --dataset /tmp/lines_demo --limit 25 --epochs 1 --max-steps 5` runs end-to-end on MPS.
...
training exercise under `research/khmer_crnn/` is complete but uncommitted.
Read `research/khmer_crnn/FINDINGS.md` and the tail of `docs/decisions/PROJECT_LOG.md`
(to match its section numbering + house style) first.

**Deliverables:**
1. **`.gitignore`:** add `research/khmer_crnn/runs/` (training artifacts) and
...
2. **PROJECT_LOG entry:** append a new section (next number in sequence, matching
   ...
   - Pointers to `research/khmer_crnn/FINDINGS.md` and `FINETUNING_PLAN.md`.
...
3. **Commit on a branch** (e.g. `experiment/khmer-crnn-trainer`): create the
   branch, then commit `research/khmer_crnn/*.py`, `*.md`, `pyproject.toml`,
   `uv.lock`, `.gitignore`, and `docs/decisions/PROJECT_LOG.md`. Commit message ends with:
   `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`. **Do NOT push.**

**Constraints:** never `git add` `research/khmer_crnn/runs/`, `eval/datasets/`,
...
`CLAUDE.md`, and `docs/architecture/CODE_AUDIT.md` first. This is the last outstanding
...
You are working in `/Users/vin/Internship/khmer-ocr-pipeline`. Read
`research/khmer_crnn/FINETUNING_PLAN.md` **§3b and §7a**, `docs/architecture/CONTEXT.md`
...
4. **Eval:** run the new engine vs the Surya baseline on the real docs through the
   existing harness — `tools/eval_document.py "<stem>" --preprocess` for both —
