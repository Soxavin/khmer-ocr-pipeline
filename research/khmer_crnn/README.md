...
uv run python research/khmer_crnn/train.py --limit 512 --epochs 1 --max-steps 5

# The benchmark (the primary deliverable) — real epoch time on this machine
uv run python research/khmer_crnn/train.py --limit 10000 --epochs 2

# Ablations to learn from
uv run python research/khmer_crnn/train.py --rnn lstm          # vs default gru
uv run python research/khmer_crnn/train.py --backbone resnet18 # lighter/faster
uv run python research/khmer_crnn/train.py --batch-size 32     # sweep batch size
uv run python research/khmer_crnn/train.py --no-augment        # augmentation off
```
...
| `--out-dir` | `research/khmer_crnn/runs` | per-run checkpoints + metrics land here |
